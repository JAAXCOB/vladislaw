...
Тебе приходят сообщения из рабочего группового чата в мессенджере. Нас интересуют ТОЛЬКО
сообщения, где сотрудник явно сообщает, что заявка ЗАКРЫТА / РАБОТА ВЫПОЛНЕНА. Всё остальное —
не наша задача, даже если оно про эвакуацию (например, ещё не завершённая работа).

Твоя задача — извлечь структурированные данные и вернуть ТОЛЬКО валидный JSON без пояснений.

ПЕРВЫМ ДЕЛОМ определи is_closed_job_report:
- true — ТОЛЬКО если сообщение явно говорит, что ЗАЯВКА (работа/заказ) закрыта/выполнена.
  Формулировки бывают очень разговорными и корявыми, распознавай смысл, а не точную фразу.
  Примеры того, что значит "заявка закрыта": "заявка закрыта", "закрыл", "сдал", "выполнено",
  "готово", "отработали", "все ок сдано", "забрал и отвёз", "поставил на стоянку", "довёз",
  "закрыта заявка", "затарил на стоянку", "все, привезли", "выполнил заказ" и любые похожие
  по смыслу формулировки о завершении РАБОТЫ.
- false — для всего остального, В ТОМ ЧИСЛЕ:
  - сообщение не про эвакуацию (приветствие, вопрос, флуд, посторонний разговор)
  - работа ещё идёт / не завершена ("еду забирать", "выехал", "принял заявку", "в пути", "гружу")
  - ВАЖНО: слово "закрыта"/"закрыл" относится не к заявке, а к чему-то другому — например
    "машина закрыта" (означает что у автомобиля заперты двери, это НЕ отчёт о завершении
    работы!), "дверь закрыта", "шлагбаум закрыт" и т.п. Не путай физическое состояние
    предмета с завершением заявки.
  - непонятно, завершена работа или нет
  Если false — все остальные поля оставь null/пустыми, needs_review=false, status="unknown".

Правила (применяются только когда is_closed_job_report=true):
...
def extract_job(message_text: str, sender_name: str = "") -> ExtractedJob:
    ...
    log.info(
        "EXTRACTED | is_closed=%s | plate=%s | make=%s %s | status=%s | confidence=%s | needs_review=%s | missing=%s",
