...
if($type==='client'){
 ...
 if(!empty($record['verification_required']))av_json_response(array('ok'=>true,'status'=>'verification_required','account_type'=>'user','id'=>$id,'verification_token'=>$record['verification_token'],'next_channel'=>'phone','message'=>'Подтвердите телефон и почту.'));
...
if($type==='executor'){
 ...
 if(!empty($worker['verification_required']))av_json_response(array('ok'=>true,'status'=>'verification_required','account_type'=>'executor','id'=>$id,'verification_token'=>$worker['verification_token'],'next_channel'=>'phone','message'=>'Фотографии сохранены. Подтвердите телефон и почту, затем заявка поступит на модерацию.'));
...
if($type==='b2b'){
 ...
 if(!empty($record['verification_required']))av_json_response(array('ok'=>true,'status'=>'verification_required','account_type'=>'b2b','id'=>$id,'verification_token'=>$record['verification_token'],'next_channel'=>'phone','message'=>'Подтвердите телефон и почту, затем заявка поступит на модерацию.'));
