...
$record=av_verification_record($type,$id);av_json_response(array('ok'=>true,'phone_verification_enabled'=>av_phone_verification_enabled(),'phone_verified'=>!empty($record['phone_verified_at']),'email_verified'=>!empty($record['email_verified_at']),'complete'=>av_verification_complete($record),'next_channel'=>av_verification_next_channel($record),'phone_masked'=>av_verification_mask($record['phone']??'','phone'),'email_masked'=>av_verification_mask($record['email']??'','email')));
