...
<select id="payment_method"><option value="cash">Наличные</option><option value="sbp" <?php echo empty($paySettings['acquiring_enabled'])?'disabled':'';?>>СБП<?php echo empty($paySettings['acquiring_enabled'])?' — после подключения банка':'';?></option><option value="card" <?php echo empty($paySettings['acquiring_enabled'])?'disabled':'';?>>Банковская карта<?php echo empty($paySettings['acquiring_enabled'])?' — после подключения банка':'';?></option></select>
